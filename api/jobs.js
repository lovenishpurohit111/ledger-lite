...
- confidence: 0.95+ for clearly visible text, 0.7-0.85 for uncertain/blurry text
- Skip percentage-only rows (OPM%, Tax%, EPS, Dividend Payout)
- Extract ALL years/columns as separate statements if multiple periods are shown, or use the most recent if there are too many`;
  ...
  try {
    ...
    throw new Error(`Gemini returned unparseable response: ${clean.slice(0, 300)}`);
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Process all uploaded files, merge results
    const allStatements = [];
    for (const file of files) {
      const base64 = file.data.toString("base64");
      const mimeType = file.type?.startsWith("image/") ? file.type : "image/png";
      const extracted = await extractWithGemini(base64, mimeType);
    ...
    // Basic validation: flag rows with low confidence
      ...
      for (const row of stmt.rows) {
        if (row.confidence < 0.8) {
          validationIssues.push({
            code: "LOW_CONFIDENCE",
            severity: "medium",
            message: `"${row.label}" has low OCR confidence (${Math.round(row.confidence * 100)}%). Please verify.`
          });
        }
      }
    }
    ...
    res.status(200).json({
      job_id: randomUUID(),
      status: "completed",
      message: `Extracted ${allStatements.length} statement(s) using Gemini AI.`,
      statements: allStatements,
      validation: {
        issues: validationIssues,
        summary: { high: 0, medium: validationIssues.length, low: 0 }
      },
      pipeline: [
        { name: "Upload", status: "completed" },
        { name: "Gemini AI Extraction", status: "completed" },
